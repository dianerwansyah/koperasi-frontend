...
export default function FormDataPinjaman() {
  ...
  const filterConfig = [
    ...
    { key: "tanggal", label: "Tanggal Pengajuan", type: "date" },
    {
      key: "status",
      label: "Status",
      type: "select",
  ...
  const handleSubmitModal = async () => {
    ...
    setSubmitting(true);
    try {
      await api.post(`/api/loans/${selectedItem.id}`, {
      ...
      showErrorToast("Gagal mengubah status pinjaman");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div data-theme="light" className="p-6 bg-base-200 min-h-screen">
      <h2 className="text-2xl font-bold mb-6">Data Pinjaman</h2>
      ...
      {selectedItem && (
        <ModalForm
          title={`Action Pinjaman - ${selectedItem.user?.name}`}
          inputs={modalInputs}
          values={modalValues}
